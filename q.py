...
def labels(my_email, account_number=1):
    ...
    try:
        ...
        for label in label_list:
            ...
            if label_name.startswith("Applications/") and label_name != "Applications/Received" and label_name != "Applications/test":
                ...
                for msg in messages:
                    ...
                    if thread_id:
                        unique_threads.add(thread_id)  # Add only unique thread IDs
            ...
            elif label_name == "Applications/test":
                # Get total messages normally for Applications/test
                label_details = service.users().labels().get(
                    userId="me", id=label["id"]
                ).execute()
        ...
        if label_dict:
            print(f"Labels and Message Count for account {account_number}: {label_dict}")
        else:
        ...
        print(f"An error occurred while processing account {account_number}: {error}")
...
def format():
    ...
    applications_received = label_account2.get('Applications/Received', 0)
    applications_rejected = label_account2.get('Applications/Rejected', 0)
    applications_interview = label_account2.get('Applications/Interview/1st Round',label_account2.get('Applications/Interview', 0))
    second_round = label_account2.get('Applications/Interview/2nd Round', 0)
    third_round = label_account2.get('Applications/Interview/3rd Round', 0)
    offers = label_account2.get('Applications/Offer', 0)
    accepted_offers = label_account2.get('Applications/Accepted', 0)
    ...
    print(third_round,"DAMEER")
    # Build the Sankey flow structure
    sankey.append(f"Applications [{applications_interview}] 1st Interviews")
    sankey.append(f"1st Interviews [{firstrejection}] Rejected")
    sankey.append(f"1st Interviews [{second_round}] 2nd Interviews")
    sankey.append(f"1st Interviews [{offers}] Offers")

    sankey.append(f"Applications [{applications_rejected}] Rejected")
    sankey.append(f"Applications [{no_answer}] No answer")

    sankey.append(f"2nd Interviews [{secondrejection}] Rejected")
    sankey.append(f"2nd Interviews [{third_round}] 3rd Interviews")

    sankey.append(f"3rd Interviews [{thirdrejection}] Rejected")  # Now correctly calculated

    sankey.append(f"Offers [{accepted_offers}] Accepted")
    sankey.append(f"Offers [{declined_offers}] Declined")

    # Print the Sankey format
    for line in sankey:
        print(line)
...
# format()
# labels(1)
#labels("da23ali83@gmail.com",1)
#labels("ali00378@umn.edu",2)
